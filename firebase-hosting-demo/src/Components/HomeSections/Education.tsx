...
class Education extends React.PureComponent<Props>{
    render() {
        ...
        var classes_taken:JSX.Element[] = [];

        for (let i = 0; i < degrees.length; i++) {
            const img = process.env.PUBLIC_URL + "/" + images[i];
            const short = shorthands[i];
            const title = titles[i];
            const degree = degrees[i];
            const context = times_and_places[i];
            const description = descriptions[i].map(descriptor => <p key={"school-descriptor-"+short+descriptor.substring(0,3)}>{descriptor}</p>);
